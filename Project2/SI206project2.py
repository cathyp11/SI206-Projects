...
def find_urls(s):
    ...
    urls = []
    for line in st:
        # find all the urls that have alphabets and numbers
        # only find urls that include at least one "." and each "." that are followed by at least 2 characters
    	urls += re.findall(r'http[s]?://[a-zA-z.]*\.[$a-z]{2,}', line)
    return urls
...
def grab_headlines():
    ...
    read = ""
    most = []
    # find the tags with class = 'view-most-read' to find the Most Read articles
    for headline in soup.div.findAll(class_='view-most-read'):
        # make a string of all the titles that are divided up by '\n'
...
def get_umsi_data():
    ...
    # iterate through all 12 pages on the website
    for i in range(13):
        # add the page numbers at the end of base_url
        url = base_url + '&page=' + str(i)
        html = requests.get(url, headers={'User-Agent': 'SI_CLASS'})
        soup = BeautifulSoup(html.text, 'html.parser')
        # find those tags with class='field-name-title' for the name of the person
        # create a list of the names
        for name in soup.div.findAll(class_='field-name-title'):
            names.append(name.text)
        # find those tags with class='field-name-field-person-titles' for the title of the person
        # create a list of the titles
        for title in soup.div.findAll(class_='field-name-field-person-titles'):
            titles.append(title.text)
    # zip the names and the titles to create a dictionary
...
def num_students(data):
    ...
    count = 0
    # iterate through the values of the dictionary, which refers to the titles
    for person in data.values():
        # check if titles == 'PhD student' and increase count if it does
