...
def get_user_tweets(user):
	if user in CACHE_DICTION:
		...
		print('fetching')
		data = api.user_timeline(user, count = 21)
...
for i in umich_tweets:
	ids = i['user']['id']
	names = i['user']['screen_name']
	favs = i['user']['favourites_count']
	des = i['user']['description']
	# looping to find users that are mentioned in tweets
...
for a in umich_tweets:
	...
	# ignore if tweet_id already exists
	cur.execute('''INSERT or IGNORE INTO Tweets (tweet_id, text, user_posted, time_posted, retweets) VALUES (?, ?, ?, ?, ?)''', (tweet_ids, texts, user_posts, times, retweets))
